...
const FilterPage = () => {
  ...
  const filterOptionsHandle = (name: string) => {
    console.log(name)
  }
  ...
  return (
    <div className="flex flex-col gap-4 w-full md:w-72 py-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <Button variant="outline" className="text-xs cursor-pointer">Clear All</Button>
      </div>
      <div className="flex flex-col">
        {
          filterOptions.map((option) => (
            <div key={option.id} className="flex items-center gap-2 mb-2 w-full">
              <Checkbox onClick={() => filterOptionsHandle(option.name)} id={`filter-${option.id}`} className="h-4 w-4 cursor-pointer" />
